...
set_handoff_template_profile() {
  ...
  HANDOFF_EXPECTED_ROLE="Evidence-bound advisory memo: findings, scoped recommendations, and explicit unknowns."
  HANDOFF_TRUTH_ANCHOR='Use provided `G-Codex-brain/ROADMAP.md` excerpt + supplied repo evidence.'
  HANDOFF_PREFLIGHT_STEP='Do not assume local shell access. If supplied repo status evidence is missing or stale, stop and return an evidence-gap request for a local executor refresh (`git status --short --branch`).'
  HANDOFF_CAN_ASSUME_LINES=$'- Supplied evidence inventory: handoff mission/constraints, pasted excerpts, and explicitly provided repo-status snippets.\n- Coordination overlay excerpts from `G-Codex-brain/`.'
  HANDOFF_NOT_ASSUME_LINES=$'- Facts not present in supplied evidence; label unknowns instead of inferring.\n- Local filesystem write access or deterministic local test execution in this environment.'
  ...
  case "${destination}" in
    ...
    GitHub*|GH*)
      ...
      HANDOFF_EXPECTED_ROLE="Evidence-bound GitHub advisory note (link-backed findings + scoped recommendations), not local execution."
      HANDOFF_TRUTH_ANCHOR='Use linked GitHub refs plus `G-Codex-brain/ROADMAP.md` excerpt from the source repo.'
      HANDOFF_PREFLIGHT_STEP='Use supplied branch/status evidence from GitHub or request a local executor refresh when missing; stop if branch or PR evidence cannot be verified.'
      HANDOFF_CAN_ASSUME_LINES=$'- Supplied evidence inventory: linked GitHub repo/PR/issue refs, visible diff/discussion context, and provided branch/status snapshot.\n- Structured mission and acceptance criteria in handoff.'
      HANDOFF_NOT_ASSUME_LINES=$'- Unlinked or unverified repository facts; report as unknown and request evidence.\n- Direct local shell execution or guaranteed writable local repository access.'
      ;;
    Cloud*|*Cloud\ Coding*)
      HANDOFF_TEMPLATE_NAME="Cloud Coding Handoff"
      HANDOFF_EXPECTED_ROLE="Bounded cloud execution report: scoped implementation notes, verification outcomes, and environment-difference notes."
      HANDOFF_TRUTH_ANCHOR='Use pinned branch/ref + `G-Codex-brain/ROADMAP.md` intent + required acceptance checks.'
      HANDOFF_PREFLIGHT_STEP='Confirm branch/ref and workspace state first; if branch/ref pin, scope, or acceptance checks are missing, stop and request clarification before execution.'
      HANDOFF_CAN_ASSUME_LINES=$'- Supplied evidence inventory: pinned branch/ref, scoped mission, and listed acceptance checks.\n- Isolated cloud workspace execution capability for scoped code changes.'
      HANDOFF_NOT_ASSUME_LINES=$'- Missing or unverified local-environment facts; call out unknowns instead of inferring parity.\n- Implicit access to local machine processes or unpublished state.'
      ;;
  esac
}
